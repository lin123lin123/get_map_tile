...
bool http_d::start_down_load(QString url,QString path,QString qt_path)
{
    if(path!="")
    {
       file =new QFile(path);
...
void http_d::loadProgress(qint64 bytesSent, qint64 bytesTotal)
 {
 qDebug()<<"3333333333333333333";
   progess=bytesSent;
  qDebug()<<"d--progess"<<progess;
   qDebug()<<"d--bytesTotal"<<bytesTotal;
     // emit write_log("loadProgress---file_name="+file->fileName()+";progess="+QString::number(progess)
     //               +";bytesTotal="+QString::number(bytesTotal));
 }
void http_d::readContent()
{
    QByteArray tmp=reply->readAll();
    buffer+=tmp;
}
void http_d::replyFinished(QNetworkReply*)
{
...
void http_d::loadError(QNetworkReply::NetworkError)    //传输中的错误输出
{
    if(file!=NULL)
    {
...
void http_d::delaymsec(int msec)
{
